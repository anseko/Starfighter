...
namespace Client.UI
{
    public class OrdersScript : NetworkBehaviour
    {
        ...
        private List<OrderUnit> _poiList;
        ...
        [SerializeField] private PoiSpawner _poiSpawner;
        ...
        private void Awake()
        {
            ...
            _poiList = new List<OrderUnit>();
            _poiSpawner = FindObjectOfType<PoiSpawner>();
        }
        ...
        private void CreatePOIFrame(OrderUnit unit)
        {
            var targetFrame = _poiList.FirstOrDefault(x => x.identifier == unit.identifier);
            isPOI = false;
            unit.operation = targetFrame is null ? OrderOperation.Add : OrderOperation.Edit;
            _poiSpawner.HandleUnitServerRpc(unit, NetworkManager.Singleton.LocalClientId);
        }

        public void AfterSpawnPoi(OrderUnit unit)
        {
            unit.orderPlane = GetNetworkObject(unit.networkObjectId).gameObject;
            _poiList.Add(unit);
            Destroy(_orderPlaneCopy);
            _orderPlaneCopy = unit.orderPlane;
        }
        
        private void DeletePOIFrame(string unit)
        {
            var poiUnit = _poiList.FirstOrDefault(x => x.identifier == unit);
            if (poiUnit is null) return;
            poiUnit.operation = OrderOperation.Remove;
            _poiSpawner.HandleUnitServerRpc(poiUnit, NetworkManager.Singleton.LocalClientId);
            _poiList.Remove(poiUnit);
        }
        ...
        public void CreateOrder()
        {
            if (isPOI)
            {
                var poiFrameInit = _orderPlaneCopy.GetComponent<POIFrameInit>();
                var poiTransmision = new OrderUnit()
                {
                    identifier = poiFrameInit.identificator,
                    position = poiFrameInit.position,
                    size = poiFrameInit.size,
                    text = _textField.text,
                    orderPlane = _orderPlaneCopy,
                    networkObjectId = _orderPlaneCopy.GetComponent<NetworkObject>().NetworkObjectId
                };
                CreatePOIFrame(poiTransmision);
            }
            else
            {
        ...
        public void CancelOrder()
        {
            ...
            {
                var thisPlane = _orderPlaneCopy.GetComponent<POIFrameInit>().identificator;
                DeletePOIFrame(thisPlane);
            }
